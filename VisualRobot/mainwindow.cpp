...
void MainWindow::on_GetLength_clicked()
{
    ...
    if (!m_hasCroppedImage)
    {
        ...
        // 检长完成后弹出缺陷检测提示
        QMessageBox::StandardButton reply = QMessageBox::question(this, "检长完成", 
            "检长已完成，是否继续完成缺陷检测？", 
            QMessageBox::Yes | QMessageBox::No);
        
        if (reply == QMessageBox::Yes) {
            AppendLog("用户选择继续缺陷检测", INFO);
            // 执行缺陷检测
            PerformDefectDetectionAfterLengthMeasurement();
        } else {
            AppendLog("用户选择不进行缺陷检测", INFO);
        }
    }
...
// 检长后的缺陷检测功能实现
void MainWindow::PerformDefectDetectionAfterLengthMeasurement()
{
    AppendLog("开始执行检长后的缺陷检测", INFO);
    
    // 读取原始图像
    Mat originalImage = imread("/home/orangepi/Desktop/VisualRobot_Local/Img/capture.jpg");
    if (originalImage.empty()) 
    {
        AppendLog("无法读取原始图像进行缺陷检测", ERROR);
        return;
    }
    
    // 获取检长过程中检测到的目标旋转矩形
    vector<RotatedRect> targetRects = GetTargetRotatedRectsFromLengthMeasurement();
    if (targetRects.empty()) 
    {
        AppendLog("未找到检长目标，无法进行缺陷检测", WARNNING);
        return;
    }
    
    AppendLog(QString("找到 %1 个检长目标，开始缺陷检测").arg(targetRects.size()), INFO);
    
    // 执行边缘检测和缺陷分析
    Mat resultImage;
    PerformEdgeDetectionOnTargets(targetRects, resultImage);
    
    // 保存并显示结果
    if (!resultImage.empty()) 
    {
        imwrite("../detectedImg.jpg", resultImage);
        AppendLog("缺陷检测结果已保存: ../detectedImg.jpg", INFO);
...
vector<RotatedRect> MainWindow::GetTargetRotatedRectsFromLengthMeasurement()
{
    vector<RotatedRect> targetRects;
    
    // 这里需要从检长结果中获取目标的最小外接矩形
    // 由于检长结果存储在DIP模块中，我们需要重新处理图像来获取这些信息
    // 或者修改DIP模块以返回旋转矩形信息
    
    // 临时实现：重新读取图像并检测目标
    Mat inputImage = imread("/home/orangepi/Desktop/VisualRobot_Local/Img/capture.jpg");
    if (inputImage.empty()) 
    {
        return targetRects;
    }
    ...
    {
        cvtColor(inputImage, source, COLOR_BGR2GRAY);
    } 
    else 
    {
        source = inputImage;
    ...
    // 二值化
    Mat binary = source > params.thresh;
    binary = 255 - binary;
    ...
    for (const auto& contour : contours) 
    {
        if (contourArea(contour) >= params.areaMin) 
        {
            ...
            targetRects.push_back(rotatedRect);
        }
    }
...
void MainWindow::PerformEdgeDetectionOnTargets(const vector<RotatedRect>& targetRects, Mat& resultImage)
{
    // 读取原始图像
    Mat originalImage = imread("/home/orangepi/Desktop/VisualRobot_Local/Img/capture.jpg");
    if (originalImage.empty()) 
    {
        AppendLog("无法读取原始图像进行边缘检测", ERROR);
        return;
    }
    ...
    // 图像预处理：高斯模糊
    int actualKsize = max(1, blurKsize * 2 + 1);
    double sigma = max(0.1, blurSigma * 0.5);
    Mat blurred;
    GaussianBlur(grayImage, blurred, Size(actualKsize, actualKsize), sigma);
    
    // Canny边缘检测
    Mat edges;
    Canny(blurred, edges, lowThreshold, highThreshold, 3);
    
    AppendLog("边缘检测完成，开始分析目标缺陷", INFO);
    
    // 创建结果图像
    resultImage = originalImage.clone();
    
    // 对每个目标进行缺陷分析
    for (size_t i = 0; i < targetRects.size(); i++) 
    {
        const RotatedRect& targetRect = targetRects[i];
        bool hasDefect = HasDefectInTarget(targetRect, edges);
        
        // 根据缺陷状态绘制目标
        DrawTargetWithDefectStatus(targetRect, i + 1, hasDefect, resultImage);
        
        // 记录结果
        if (hasDefect) 
        {
            AppendLog(QString("目标%1: 检测到缺陷").arg(i + 1), WARNNING);
        } 
        else 
        {
    ...
    AppendLog(QString("缺陷分析完成，共分析 %1 个目标").arg(targetRects.size()), INFO);
}

bool MainWindow::HasDefectInTarget(const RotatedRect& targetRect, const Mat& edges)
{
    // 获取旋转矩形的边界框
    Rect boundingBox = targetRect.boundingRect();
    
    // 确保边界框在图像范围内
    boundingBox = boundingBox & Rect(0, 0, edges.cols, edges.rows);
    ...
    // 提取目标区域内的边缘图像
    Mat targetEdges = edges(boundingBox).clone();
    
    // 创建目标区域的掩码
    Mat mask = Mat::zeros(targetEdges.size(), CV_8UC1);
    ...
    for (int i = 0; i < 4; i++) 
    {
        Point pt(static_cast<int>(vertices[i].x - boundingBox.x), 
                 static_cast<int>(vertices[i].y - boundingBox.y));
        polygonPoints.push_back(pt);
    }
    
    // 填充多边形掩码
    fillConvexPoly(mask, polygonPoints, Scalar(255));
    
    // 应用掩码，只保留目标区域内的边缘
    Mat maskedEdges;
    bitwise_and(targetEdges, targetEdges, maskedEdges, mask);
    
    // 计算目标区域内的边缘像素数量
    int edgePixelCount = countNonZero(maskedEdges);
    
    // 计算目标区域的面积
    double targetArea = targetRect.size.width * targetRect.size.height;
    
    // 计算边缘密度
    double edgeDensity = static_cast<double>(edgePixelCount) / targetArea;
    
    // 设置阈值来判断是否有缺陷
    // 如果边缘密度超过阈值，则认为有缺陷
    double defectThreshold = 0.01; // 可根据实际情况调整
    
    return edgeDensity > defectThreshold;
}

void MainWindow::DrawTargetWithDefectStatus(const RotatedRect& targetRect, int targetIndex, bool hasDefect, Mat& resultImage)
{
    // 根据缺陷状态选择颜色
    Scalar color = hasDefect ? Scalar(0, 0, 255) : Scalar(0, 255, 0); // 红色有缺陷，绿色无缺陷
    ...
    for (int i = 0; i < 4; i++) 
    {
        line(resultImage, vertices[i], vertices[(i + 1) % 4], color, 3);
    }
    
    // 绘制序号
    Point2f center = targetRect.center;
    string indexText = to_string(targetIndex);
    int fontFace = FONT_HERSHEY_SIMPLEX;
    ...
    // 计算文本大小
    int baseline = 0;
    Size textSize = getTextSize(indexText, fontFace, fontScale, thickness, &baseline);
    
    // 计算文本位置（矩形中心）
    Point textOrg(static_cast<int>(center.x - textSize.width / 2), 
                  static_cast<int>(center.y + textSize.height / 2));
    
    // 绘制文本背景
    rectangle(resultImage, 
              Point(textOrg.x - 2, textOrg.y - textSize.height - 2),
              Point(textOrg.x + textSize.width + 2, textOrg.y + 2),
              color, -1);
    
    // 绘制序号文本
    putText(resultImage, indexText, textOrg, fontFace, fontScale, Scalar(255, 255, 255), thickness);
    
    // 如果有缺陷，在矩形框旁边添加"缺陷"标记
    if (hasDefect) 
    {
        string defectText = "缺陷";
        Size defectTextSize = getTextSize(defectText, fontFace, fontScale, thickness, &baseline);
        Point defectTextOrg(static_cast<int>(center.x - defectTextSize.width / 2), 
                           static_cast<int>(center.y + textSize.height + defectTextSize.height + 10));
        
        // 绘制缺陷文本背景
        rectangle(resultImage, 
                  Point(defectTextOrg.x - 2, defectTextOrg.y - defectTextSize.height - 2),
                  Point(defectTextOrg.x + defectTextSize.width + 2, defectTextOrg.y + 2),
                  Scalar(0, 0, 255), -1);
        
        // 绘制缺陷文本
        putText(resultImage, defectText, defectTextOrg, fontFace, fontScale, Scalar(255, 255, 255), thickness);
    }
}
}
