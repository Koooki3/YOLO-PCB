...
ClassificationResult DLProcessor::postProcessClassification(const std::vector<cv::Mat>& outs)
{
    ...
    try {
        ...
        // 对于二分类，输出可能是单个值（sigmoid）或两个值（softmax）
        if (output.total() == 1) {
            // 单输出sigmoid情况
            float confidence = output.at<float>(0);
            result.classId = (confidence > confThreshold_) ? 1 : 0;
            result.confidence = (result.classId == 1) ? confidence : (1.0f - confidence);
...
bool DLProcessor::isModelLoaded() const
{
    return isModelLoaded_;
}

std::vector<std::string> DLProcessor::getClassLabels() const
{
    return classLabels_;
}

cv::Size DLProcessor::getInputSize() const
{
    return inputSize_;
}
