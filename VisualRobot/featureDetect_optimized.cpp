    ...
    if (!params.enableParallel || params.numThreads <= 1) {
        ...
        for (int i = 0; i < numThreads; ++i) {
            ...
            if (start1 < keypoints1.size()) {
                threads.emplace_back(ParallelKeypointFilter, 
                    std::cref(keypoints1), std::ref(validKeypoints1), 
                    params.responseThresh, start1, end1);
            }
            
            if (start2 < keypoints2.size()) {
                threads.emplace_back(ParallelKeypointFilter, 
                    std::cref(keypoints2), std::ref(validKeypoints2), 
                    params.responseThresh, start2, end2);
            }
        }
        ...
        for (int i = 0; i < numThreads; ++i) {
            goodMatches.insert(goodMatches.end(), 
                threadGoodMatches[i].begin(), threadGoodMatches[i].end());
            points1.insert(points1.end(), 
                threadPoints1[i].begin(), threadPoints1[i].end());
            points2.insert(points2.end(), 
                threadPoints2[i].begin(), threadPoints2[i].end());
        }
    }
...
FeatureDetectorOptimized::ParallelResult FeatureDetectorOptimized::ProcessImagePair(
    ...
    const FeatureParams& params)
{
    ...
    // 并行图像预处理和特征提取
    std::future<cv::Mat> futureStandardized1 = std::async(std::launch::async, 
        [&dataProcessor, image1]() { return dataProcessor.standardizeImage(image1); });
    std::future<cv::Mat> futureStandardized2 = std::async(std::launch::async, 
        [&dataProcessor, image2]() { return dataProcessor.standardizeImage(image2); });
    ...
    // 并行匹配筛选
    result.matches = FilterMatchesParallel(keypoints1, keypoints2, knnMatches, 
        result.points1, result.points2, params);
    
    result.success = true;
    return result;
}

std::vector<FeatureDetectorOptimized::ParallelResult> FeatureDetectorOptimized::BatchFeatureDetection(
    ...
    const FeatureParams& params)
{
    ...
    for (const auto& pair : imagePairs) {
        futures.push_back(std::async(std::launch::async, ProcessImagePair, 
...
std::future<FeatureDetectorOptimized::ParallelResult> FeatureDetectorOptimized::AsyncFeatureDetection(
    const QString& imagePath1, 
    const QString& imagePath2,
    const FeatureParams& params)
{
