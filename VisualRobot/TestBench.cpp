    ...
    int main(int argc, char** argv)
    {
        ...
        if (!loaded) {
            ...
            for (size_t i = 0; i < fileList.size(); ++i) {
                ...
                if (fileList[i].find("_def") != string::npos) labs.push_back(1);
                else labs.push_back(0);
            }
            if (feats.empty()) {
        ...
        while (true) {
            ...
            int id = -1;
            try { id = stoi(line); } catch (...) { cout << "非法输入\n"; continue; }
            if (id < 0 || id >= (int)fileList.size()) { cout << "编号越界\n"; continue; }

            Mat img = imread(fileList[id]);
            if (img.empty()) { cout << "读取图片失败：" << fileList[id] << "\n"; continue; }
